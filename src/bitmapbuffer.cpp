...
bool BitmapBuffer::liangBarskyClipper(coord_t & x1, coord_t & y1, coord_t & x2, coord_t & y2, coord_t xmin, coord_t xmax, coord_t ymin, coord_t ymax)
{
  // defining variables
  float p1 = -(x2 - x1);
  float p2 = -p1;
                            ...
                            uint8_t pat, LcdFlags flags)
{
  ...
  if (!liangBarskyClipper(x1, y1, x2, y2, xmin, xmax, ymin, ymax))
    return;
